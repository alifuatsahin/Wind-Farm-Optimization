...
def get_local_velocity_field(config, wind_farm, field_params):
    ...
    U_wake, V_wake, W_wake, _ = momentum_conserving_superposition(
        U_list, V_list, W_list, U_inf=config.Uin
    )

    # Total local wind = freestream + wake-induced perturbation
    U_total = U_base + U_wake
    V_total = V_base + V_wake
    W_total = W_base + W_wake

    return U_total, V_total, W_total

def momentum_conserving_superposition(U_list, V_list, W_list, U_inf, max_iter=10, tol=1e-3):
    """
    ...
    # Individual velocity deficits u_i_s = U_inf - U_i
    u_s_list = [U_inf - U for U in U_list]

    # --- Initial convection velocity estimate: Uc_i = mean(U_i) ---
    # Correct version: convection velocity = mean local wind (not deficit!)
    ...
    for _ in range(max_iter):
        ...
        Uw_total = U_inf - Us_total  # reminder: Us = U_inf - U
